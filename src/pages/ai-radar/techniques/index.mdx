...
<div data-radar data-label="Classical ML" data-ring="adopt" data-change='new' />
...
[Bidirectional Encoder Representations from Transformers (BERT)](https://huggingface.co/docs/transformers/en/model_doc/bert) revolutionised Natural Language Processing (NLP) by allowing AI models to process human language by looking at words in relation to their entire context, rather than just left-to-right or right-to-left. Think of it like a reader who can understand a word by looking at all the surrounding words for context, rather than reading sequentially. The original BERT spawned a family tree of variants and new versions are still being developed, most recently [ModernBERT](https://huggingface.co/blog/modernbert), which is too new at the time of writing for us to have independently evaluated.
