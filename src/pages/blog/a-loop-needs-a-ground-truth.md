...
<span class="pullquote" text-content="Hand the inner loop to an agent and the head goes with it."></span>That exit condition is doing something subtle. Ordinary test-driven development is a tight inner loop: implement, run tests, repeat, driving *code* toward whatever the tests currently say. The tests evolve too, of course — every practitioner reworks them as understanding deepens — but that outer movement happens in the developer's head, with nothing checking it. Hand the inner loop to an agent and the head goes with it. Allium's loop keeps the outer arc and makes it explicit: the verify phase can feed back into the *spec itself*. When a test reveals that the intent was ambiguous, or `weed` shows the spec, not the code, is the thing that's wrong, you re-enter at gather-context, fix the spec, and re-project the tests. The inner arc converges code to the contract; the outer arc converges the contract to what was actually wanted. It's exactly the move a pipeline cannot make — and exactly the arc missing from the story I opened with, where nothing converged the tests to the intent.
