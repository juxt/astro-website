...
Isolation keeps the verdicts honest too, because nobody marks their own homework. The tests are executed and parsed rather than narrated, and `weed` judges spec-code alignment as a separate agent in its own clean context. The agent that wrote the code never gets to declare the goal met. And because the phases talk to each other only through artefacts on disk (the spec, the tests, the code, and a small ledger), never through state held in memory, a run can be resumed if it's interrupted.
