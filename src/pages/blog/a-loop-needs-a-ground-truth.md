...
**Delegate, don't inline.** The loop doesn't reimplement the phases; it delegates each one to the skill or agent that already does that job. That's the DRY choice, but there's a sharper reason: because each phase runs in its own context, the orchestrator holds only the loop state while the heavy work happens elsewhere and comes back as a result. That context isolation pays twice. It keeps a long, many-tick run inside a sane token budget, and it keeps the model sharp: models reason worse as their context fills, so a phase that starts clean, reading only the artefacts it needs, makes better decisions than one wading through the residue of every tick before it. It also means nobody marks their own homework. The tests are executed and parsed rather than narrated, and `weed` judges spec-code alignment as a separate agent in its own clean context; the agent that wrote the code never gets to declare the goal met. The phases talk to each other only through artefacts on disk (the spec, the tests, the code, and a small ledger), never through state held in memory, which is also what makes a run resumable if it's interrupted.
