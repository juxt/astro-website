...
Allium is our attempt at that spectroscope: a way to read composition at a distance from the code. The distance between intent and implementation, which the agile founders sought but never had the tools to sustain, turns out to be the space worth iterating in.
