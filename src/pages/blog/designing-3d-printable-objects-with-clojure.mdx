...
title: 'Unleashing Creativity: Designing 3D Printable Objects with Clojure and OpenSCAD'
description: "Empowering Clojurians with the Power of 3D printing"
...
Discover the exciting world of 3D printing and harness the versatility of Clojure, a
powerful programming language, to design and create your very own 3D printable objects.
In this blog post, we dive into the fusion of art, technology, and coding, exploring the
endless possibilities and efficiency of using Clojure for 3D design.
Whether you're a seasoned programmer or an aspiring maker, get ready to unleash your creativity and bring your
imaginative ideas to life through the magic of 3D printing and Clojure.

## The Intersection of Art and Engineering: An Introduction to 3D Printing Design:

The intersection of art and engineering in the realm of 3D printing design is a fascinating and dynamic field that merges creativity and
technical expertise. It allows artists and designers to push the boundaries of traditional art by harnessing the power of 3D printing technology.
Through 3D printing, artists can bring their imaginative visions to life with intricate precision, incorporating complex geometries and
structures that were once unimaginable. This collaboration between art and engineering not only opens up new space for artistic expression,
but also revolutionizes industries such as architecture, product design, and even medical
advancements, as the possibilities of 3D printing continue to expand.

So let's uncover the world of 3D modeling together.

## Setting Up Your Environment for 3D Design with Clojure and OpenSCAD:

* Install OpenSCAD (to visualize and render model in real-time): Visit the official [OpenSCAD website](https://openscad.org) and download the
...
* Clone github project [squiggle](https://github.com/herichovadajana/squiggle). When the project is cloned, execute command "clj -X:new"
to download dependencies and generate the squiggle.scad file.
* Open squiggle.scad file in OpenSCAD by running `openscad CAD/squiggle.scad` from the command line in the project directory.
...
* To run the project in Emacs, you can run it by invoking `M-x` and `cider-jack-in`. If you are using different IDE,
...
* Open playground.clj namespace. This namespace already contains everything we need to start with our design.

## From Idea to Reality: Step-by-Step Guide to Designing 3D Printable Objects:

In this blog post we will start with designing a very simple object using basic shapes. But don't worry, this can lead to stunnig creations
that are both funny and interesting. By combining spheres, cuboids, cylinders, and other elementary shapes, we can craft imaginative characters,
playful sculptures, or quirky inventions. The simplicity of the design elements adds a charm of its own. These seemingly basic shapes can be
transformed into interesting complex creations.
...
Are you ready? Let's design our first shape.
...
The [open-scad.clj](https://github.com/herichovadajana/squiggle/blob/main/src/squiggle/open_scad.clj) namespace contains all the shapes available for examination.
...
along the x-axis with a factor of 1.5 and setting the radius to 10. Additionally, we need to update the
rendering definition to use the `body` definition instead of `my-design` function.

* *`os/scale` function function is used to resize or scale a shape in three-dimensional space*
...
For this purpose, we will utilize the `os/difference` function. However, before performing the subtraction,
...
we provide a vector of dimensions "a", "b", and "c" as an argument. If we want the cuboid to be centered, we can also include the additional argument :center true.

* *`os/difference` function is used to subtract one or more shapes from another shape*

``` clojure
(def render (core/write (os/cube [100 100 10] :center true)))
```
...
To position it correctly, we will use the os/translate function. It accepts a map of positioning coordinates
...
* *`os/translate` function is used to move or translate a shape in three-dimensional space.*
...
Now it's time to incorporate the tail. We will introduce two new functions for this purpose. Firstly, the `os/rotate` function, which accepts
a map of axes and angles as arguments, allowing us to rotate an object around a specific axis. Secondly, the `os/hull` function will be used.

* *`os/hull` function is used to create a new shape that encompasses multiple input shapes. It takes multiple shapes as arguments and generates a new shape that
covers the outer boundary of all the input shapes. The resulting shape forms a convex hull that encloses the combined geometry of the input shapes.*


To design the tail more effectively and have better overview of our progress, we will create a separate definition for it.

As a foundation for the tail, we will generate a scaled sphere with a radius of 2. However, this time we will scale it
along both the :x and :y axes. (Don't forget to update the render to display the tail instead of the body.)
...
By shifting the tail towards the left (along the x-axis) and upward (along the z-axis), 
...
To change this, we can achieve a more accurate representation by rotating the tail along the x-axis.

*`os/rotate` function is used to rotate a shape in three-dimensional space*
...
Although it has improved, it is still not perfect. It's time to make some adjustments by cutting a specific shape from the duck body.
...
(def render (core/write tail-cut))
...
Now we will proceed to create a duck definition where we will start assembling the various pieces. The initial step
...
*`os/union` function is used to combine multiple shapes or geometries into a single object.*
...
Now we reach an interesting part where we will use the power of Clojure functions. We are going to create the wings for our design.
To begin, we will start with the feather component, which involves scaling, translating, and rotating a sphere shape.
...
We can visualize the feather by rendering it. And by experimenting with different feather sizes, we can determine the ideal size we need for our duck.

```clojure
(def render (core/write (feather 0 0 2 0)))
```
...
```clojure
(def render (core/write (apply os/union (row-of-feathers 5 0 1 4))))
```
To combine all the individual feathers into a single wing, we use the `(apply (os/union ...)` function. In this particular case,
we apply it onlly for rendering purposes to see if the row of wings aligns with our intended design. Additionally, we have provided a translation of
...
```clojure
(def render (core/write (apply os/union (row-of-feathers 0 0 1 4))))
```
...
(def render (core/write (create-wing {:outer-feathers-count 3
...
To incorporate a wing on the opposite side, we can utilize the `(os/mirror)` function to mirror it around the :y axis.

*`os/mirror` function is used to create a mirrored or reflected copy of a shape.*
...
To construct the duck's bill, we will utilize the `os/rotate-extrude` function. This function generates a
3D object by rotating a 2D shape around the z-axis. We will begin with the following 2D shape.

*`os/rotate-extrude` function is used to create a 3D object by revolving a 2D shape around a specified axis.
It takes a 2D shape as input and rotates it around the specified axis, typically the z-axis, to generate a solid 3D object.
The resulting shape is created by sweeping the 2D shape through a full 360-degree rotation. Additionally, the function
accepts an angle parameter to specify the desired rotation angle.*
...
To add a touch of elegance to our duck, we will give it a hat using shapes like `os/rounded-cylinder` and `os/cylinder`.
...
This function transforms the 2D shape into a 3D object by extruding it in a specified direction.
The height parameter determines the extent of the extrusion along the Z-axis.

*`os/linear-extrude` function is used to create a 3D object by extruding a 2D shape in a specified direction. It takes a
2D shape as input and extends it along the specified axis to generate a solid 3D object.*
...
To render your duck or any other object you've designed, follow these steps in OpenSCAD: navigate to `edit -> render`
...
Now you can see that by combining the elegance and expressiveness of Clojure with the innovative capabilities of 3D printing, you have the power to turn your ideas into real objects.
Whether you're a hobbyist, designer, or engineer, this blog post has equipped you with the basic knowledge and tools to start an exciting journey of 3D design using Clojure.
Let your imagination fly and start transforming your creative visions into reality.
