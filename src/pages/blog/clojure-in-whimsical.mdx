...
We've got a little bit of WebAssembly for specific problems. Since we're a canvas app so we're using got a bunch of graphical stuff too. It's shiny to me anyway, even though maybe it's less shiny to others. I think Clojure and the associated data structures are a really important part of our app. We're using AVL maps, not that that's specific to Clojure, but there's a really great library for it. We're using hiccup everywhere for a lot of things. I'd say that Clojure really does describe not just the code that we're writing, but also operationally, a lot of the things that we do are really heavily influenced by it.
