...
Clojure and the associated data structures are an important part of our app. We're using AVL maps, not that that's specific to Clojure, but there's a really great library for it. We're using [Hiccup](https://github.com/weavejester/hiccup) everywhere. I'd say that Clojure really does describe not just the code that we're writing, but also operationally, a lot of the things that we do are really heavily influenced by it.
