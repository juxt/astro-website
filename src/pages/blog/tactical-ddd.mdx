...
In this [previous](../event-driven-architecture) blog post, we attempted to map out aspects of Event Driven Architecture.  This post is exploring Domain Driven Design (A) on the EDA map.  Whilst Domain Events (B) is an important part of modern EDA having a grounding in the concepts of DDD before Domain Events are introduced is beneficial to all aspects of EDA.
...
Within an aggregate, consistency rules are applied synchronously and across boundaries (multiple aggregates) updates are asynchronous (eventually consistent).
...
Evan's has quite a bit more to say on the matter:
...
This puts us at (Loc A) in the EDA taxonomy of the [previous](../event-driven-architecture) post. From this simple domain model foundation, we can explore the addition of domain-events, command sourcing, and event sourcing.  We can pivot out the underpinnings to show the impact of using message-driven architecture or event-stores to implement the solutions.
