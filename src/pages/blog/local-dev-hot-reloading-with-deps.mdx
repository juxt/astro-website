...
In fact, I was involved by our CTO in one of our internal projects, and I was asked to build a library that would help us interact with our OAuth2 server.
I had never built a Typescript library before, so I decided to document my experience in this article.

Coming from a Clojure background, I was looking for a seemless dev experience, where I could quickly iterate on my changes and test them quickly.
I also wanted to be able to publish the library on the npm registry, so that other developers could use easily find it and use it.

### Prerequisites:

The tools I've used in this article are the following:
...
After a bit of googling, I found out about `preconstruct`, which is a tool that can help you set up a library project with ease.
...
At this stage, the `dist` folder doesn't exist yet, so you can just go ahead and copy from the following example:
...
To do this, I leveraged the `bin` field in the `package.json` file. Here's an example:
...
Of course you can create as many commands as you want and bind them to different files.
