...
```
Assuming for now that we have access to some sort of iterator interface where each iterator satisfies the contstraints the
specific `where` clause and where the data is sorted while we iterate over it.
```clj
...
the given key. The joining of multiple iterators can then be done by keeping track of the head with largest element seen so
far and trying to bring the iterator with the smallest element at the head to be equal to the one with the largest element.
If the largest and the smallest are equal we have found a match.

<Image alt='image' width='1000' src={unaryjoin} />

Describe the picture above better.
...
The following are roughly in order of (my subjective) comprehensibility:
