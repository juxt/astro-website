...
import binarytrie from '../../assets/blog/wcoj/binary-trie-named-with-ops.png'
...
in databases these days (and what for example Postgres does) is to join a relation at a time.
...
out at a set and the `:doc-store` a standard hash-map.
...
having added the above fact to an empty database the indices and document store would look as follows:
```clj
;; eav index
{1 {2 #{3}}}
;; aev index
...
### Iterators / a simple a datalog engine

Let's look at how we retrieve and iterate over our data for a where clause of the form
```clj
[?person :person/eats ?food]
```
one thing we have to decide is in which order to iterate over the variables. Let's for now fix
the order in which they first appear in the query (also what the engine is going to use). The
iteration for the above clause is best illustrated by the following diagram

<Image alt='image' width='500' src={binarytrie} />

Now that we know how things are stored we can think about how to retrieve them. Let us introduce
a tuple (TODO change name, also for subfields)
```clj
{:triple '[?e :person/eats ?v]
  :triple-order '[:e :a :v]}
```
A tuple contains the data of a where clause and the order of in which things should be returned
(it also tells us which index we have to look at).

In the beginning we were also exploring the option of using one iterator for different levels
(thereby needing a second protocol for moving up and down in the hiarachy).
One would move up and down the levels of the trie and could iterate at each level. This option
turned out to need a lot more bookkeeping. So we went for creating a new iterator each time
