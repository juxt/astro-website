...
all about. The important part is that no graph can have more then $O(N^{3/2})$ triangles
...
(essentially calculating 2-paths) you will end up with $O(N^2)$ intermediate rows. What
that means for our binary join strategy above is that no matter what binary join tree you
...
So if you fix some schema for you database and you fix some query over that schema then
