...
Each of these constrains the agent's decomposition choices. But Conway's Law operates at the organisational level too. Deliberate agent decomposition, which agent handles which domain, with what interfaces between them, is organisational design in Conway's sense, even when the committee members are processes. Strowger's switch replaced one operator on one line. The [telephone exchange](https://en.wikipedia.org/wiki/Telephone_exchange) that grew from it solved a harder problem: routing calls across a network of subscribers who didn't share a wire. The emerging [protocol ecosystem](https://modelcontextprotocol.io/) for agent communication is the same kind of infrastructure, routing work through nodes that don't share a context window.
...
Every agent swarm is building an exchange, whether its operator designs it or not. These principles of simplicity, evidence, judgement and rigour have shaped [how we approach AI-assisted engineering at JUXT](/). If you'd like help taking control of the structure, [we'd welcome a conversation](mailto:info@juxt.pro?subject=AI-assisted%20engineering).
