...
If we don't want to include the end marker, we can just add a `(take-nth 2)` step:
...
These aren't much terser, but are arguably clearer to read. However, these examples rely on a few assumptions. Let's walk through some scenarios which break these assumptions:

### Partition markers may be consecutive
Clojure's `partition-by` groups consecutive elements for which `f` has the same return value, so consecutive markers are not split. 
...
Previous examples using `partition-by` with `partition-all 2` rely on a specific order of markers and non-markers; namely that the first starting marker occurs before the first non-starting marker, and that the first ending marker occurs after the first non-ending marker. If this is starting to feel a bit overly detailed, then the point has been made: building this capability manually can uncover unexpected behaviours before too long. Let's take a look at a few examples:
...
Indeed, the stateful transducer arity of `partition-after` uses a similar approach. However, we usually try to avoid explicitly handling mutable state with otherwise-lazy code, and for good reason. This example is the least declarative, and therefore least readable of all the examples so far.
...
The function `drop` explicitly returns a lazy-seq, so it wasn't immediately obvious to me. I'm not sure it was to Weavejester either, but we decided to keep it. Maybe it prevents `drop`'s return from being realized immediately? If you have any ideas, please let me know!
...
* Fjord picture at the top from [Robert Bye](https://unsplash.com/@robertbye)
