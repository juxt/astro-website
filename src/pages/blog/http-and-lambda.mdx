...
As I investigated them I found that each had slightly different behaviours,
quirks and setup instructions. I ended up with 20+ tabs open and writing a
bunch of test cases, so I thought I would share what I had learned all in one
place.
...
- For everything else I'd suggest: [ALBs](#alb) or [API Gateway w/ REST & Lambda Proxy Integration](#api-gateway-w-rest-api--lambda-proxy-integration)
...
formats are in JSON. It is worth noting is if the response **does not** match
this format then a `200` response is assumed.
...
- [Create your CloudFront Distribution](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/distribution-web-creating-console.html)
  ...
  - For **Allowed HTTP methods** choose all the methods you plan to use (as you'll see, this is the "gotcha" I mentioned)
...
was [simple enough](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/create-application-load-balancer.html).
As ever when configuring ALBs I ran into an issue where my Security Group was
misconfigured and my tests were timing out, but once that was resolved
...
formats are similar but slightly different to the Function URL formats. One
major difference is that ALBs are more *strict* with the response format.

Because of this strictness I wanted to test the case where the response
**format** was valid too, so I modified the lambda code like so:
...
(defn -handleRequest [_ is os context]
  (let [input (slurp is)]
    ...
    (when (re-find #"error" input)
      (throw (ex-info "test" {:my "error"})))
    (cond
      (re-find #"invalid" input) (spit os "{")
      ;; 👇 Added an option to return JSON in the format that the ALB wants
      (re-find #"valid" input) (spit os "{\"isBase64Encoded\":false,\"statusCode\":200}")
      :else (spit os "{}"))))
```

And an additional test using the command: `curl -v -XPOST --data '{}' --header 'Test: valid' <url under test>`
...
obscure `502 Bad Gateway` error. So make sure to catch exceptions and always
...
but it's still worth understanding the default behaviours though to know what
you need to configure. I would suggest re-running these tests (and others!)
after making configuration changes to see how these behaviours have really
changed:
...
{/*
TODO: Remove this section and instead link to docs.
For the remaining sections:
- Check that instructions I provide only show the *specific* things I did to setup a given test
- Link lots!
*/}
