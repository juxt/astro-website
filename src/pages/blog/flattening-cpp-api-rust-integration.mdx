...
#### MessageView::to_owned() When receiving messages through a callback, the
data arrives as a MessageView reference, which provides zero-copy access to data
held by the underlying C++ API. If you need to retain the message data beyond
the callback's scope, you can call `to_owned()` to create a deep copy, which
returns an owned Message. See
