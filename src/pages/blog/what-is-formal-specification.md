...
A specification here consists of two kinds of statement. First come the rules of the design, the things you intend to build: every order belongs to exactly one customer; an order can only be confirmed against a hold on its seat; a seat carries at most one unexpired hold at a time. Then come the consequences you believe follow from those rules, such as: no seat is ever referenced by two confirmed orders. That consequence is the invariant your unique constraint enforces, and it seems so obviously implied by the rules that most design documents would never spell it out. The word doing the work in that sentence is "believe".

Checking, in this tradition, means testing whether the consequence does follow from the rules, and the method is blunt: examine every small situation the rules permit and see whether the belief survives in all of them. A "situation" is nothing exotic. It is a tiny, fully specified database state: Alice and Bob exist; seats 14A and 14B exist; there is one hold, Bob's on 14B, unexpired; there is one confirmed order, Alice's, for 14A. That is a situation. Change any detail, whose hold, which seat, expired or not, and you have a different one. [Alloy](https://alloytools.org) generates every situation that can be built within a bound you choose, say at most two customers, two seats, two holds and two orders, discards those that break your rules, and searches the rest for one that breaks your belief. Even this toy bound yields thousands of distinct situations, which is why the search is a machine's job; Alloy translates the whole question into boolean logic and a solver settles it in milliseconds.

For the rules above, the solver finds something. Seat 14B carries two holds: an expired one belonging to Alice and a current one belonging to Bob. Alice's order was confirmed against her hold, Bob's against his. Check each rule: every order belongs to one customer, every confirmed order references a hold on its seat, and 14B carries only one unexpired hold. All satisfied. Yet two confirmed orders reference 14B, because nothing in the rules says a confirmation must outlast the hold it was granted on. The belief does not follow, the design is weaker than you thought, and you learned it from a diagram of five records, before writing any code. A counterexample, in this tradition, is a picture.

What the shape view cannot express is change. The diagram shows a state that can exist; it says nothing about how the system arrives there, because a situation is a snapshot, and nothing in a snapshot records its own history. The failures we are most worried about are failures of becoming, a hold that was valid when payment began and expired before it completed, and to catch those the model needs motion.
