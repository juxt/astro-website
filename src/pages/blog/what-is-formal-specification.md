...
Read as a specification, this code makes one promise and withholds another. The guard promises that confirmation only happens while the hold is alive, which as a lasting fact about the data reads: every confirmed order was confirmed before its hold expired. And nothing here, or anywhere else in the codebase, revokes a confirmation when its hold later dies. What the code declines to do is as much a part of the specification as what it does.

Nor is your code the only author of rules. The schema contributes several: every order references exactly one hold, every hold exactly one seat. The database contributes any constraints you have declared, though notice what the seating remodel did to those: an order's seat now sits two joins away, beyond the reach of a unique index, so "one confirmed order per seat" stopped being something Postgres enforces and became something you believe. The clock contributes too, because holds expire without a line of your code running. A model in this tradition describes the whole system, your code, your schema, your dependencies and the passage of time, and building one obliges you to know the semantics of each. That obligation is not new, only newly visible. The write skew at the top of this article got through because an unexamined mental model of READ COMMITTED did not match the database's own specification; modelling turns the mental model into an artefact that can be checked.

So the rules, with their sources: every confirmed order was confirmed before its hold expired (the guard above); a seat carries at most one unexpired hold at a time (the endpoint that grants holds, which has a guard of its own); orders reference holds, and holds reference seats (the schema). The belief: no seat is ever referenced by two confirmed orders. It seems to follow. Checking means finding out, and the method is blunt: examine every small situation the rules permit and see whether the belief survives in all of them. A "situation" is a tiny, fully specified database state: Alice and Bob exist, seats 14A and 14B exist, one hold, one confirmed order, every timestamp filled in. Change any detail and you have a different one. [Alloy](https://alloytools.org) builds every situation that can exist within a bound you choose, say two customers, two seats, two holds and two orders, discards those that break a rule, and searches the remainder for one that breaks the belief. This is not fuzzing: Alloy never executes your code, and the code participates in the analysis only through the rules you transcribed from it. Even the toy bound yields thousands of situations, so the search is a machine's job. Alloy compiles the question into boolean logic and a solver settles it in milliseconds.

For our rules, the solver finds something. Seat 14B carries two holds: Alice's, which expired at 19:10, and Bob's, created at 19:11 and live until 19:21. Alice's order was confirmed at 19:05, Bob's at 19:12. Test every rule: each confirmation happened before its hold expired, the seat carries exactly one unexpired hold, every reference is in place. All satisfied, and the guard in `confirm` was honoured both times it ran. Yet two confirmed orders reference 14B. The natural objection, "my code would never allow this", runs into the timestamps: no bug was required, only the passage of six minutes between one confirmation and the next. The rules never say a confirmation must outlast its hold, because the code never enforces it. The belief does not follow, and the whole discovery is a diagram of five records. A counterexample, in this tradition, is a picture.
