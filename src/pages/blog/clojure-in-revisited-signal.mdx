...
heroImage: 'signal-ai-logo-vector.png'
clojureIn:
  {
    season: 'two',
    industry: 'finance',
...
**Rahul:** The primary use case that we have is to run this massive search cluster and we monitor this cluster at all times. We need to protect it against large queries that could overwhelm or load the cluster. Clojure allows us to walk the tree with it's standard library and to preemptively measure the impact of a query and, so we can therefore plan and adjust.
...
**Charlie:** One concern worth mentioning is libraries being maintained by individuals, who may have moved on. A good example is [Amazonica](https://github.com/mcohen01/amazonica). We still mainly reach for Amazonica when interacting with services, but it is a concern that it's maintained by one person who doesn't work on Clojure anymore and it's built using SDK, which may get dropped by Amazon at some point.
...
We tried to use Clojure spec to make our boundaries more strict. We found found it hard to work with, and now we use [Malli](https://github.com/metosin/malli) and it just seems more intuitive. We never really got to grips with how to structure our specs and actually to use them in the right context, like development Vs production. You seem to need to do a lot of work to make actually fit for human consumption.
...
**Richard:** In the Search team we're aiming for simplicity at all costs. For lifecycle we're with [Mount](https://github.com/tolitius/mount) and configuration we're using [cprop](https://github.com/tolitius/cprop) and I think that's a fairly simple and easy way of configuring your projects. We like [deps.edn](https://clojure.org/guides/deps_and_cli) for dependency management and [Cheshire](https://github.com/dakrone/cheshire) for JSON.
