...
author: 'yav'
title: 'The slow cook'
description: 'Nothing fails, everything slows, and no alert fires. Distributed platforms solved quiet degradation long ago. Agent loops need the same instrumentation.'
category: 'ai'
...
[Evals are not tests](https://www.juxt.pro/blog/evals-are-not-tests/) gave one answer, an eval on a nightly clock, watching the quality of what the agent writes. That catches a change in the output, which is not necessarily a reflection of a change in the flow. Distributed platforms have been watching the flow for decades.
