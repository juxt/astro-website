...
Valuation jobs ran as phases, each phase a set of tasks dealt out round robin through a cluster of messaging nodes to a huge pool of workers across the estate. One node passing its work on a little more slowly, or one machine behind it slowed by a hardware fault, and that share fell behind. Fan-out turned one slow link into everyone's problem, in two parts. Each job spread thousands of tasks across the pool, so every job had a slice of its tasks land on the slow node, and those small delays accumulated into a late finish for the job itself, however much ran in parallel. Then the queue compounded it. The system could only run so many jobs at once, and with the jobs in progress holding their slots for longer, everything queued behind them waited longer to even start. Nothing had failed, so nothing alerted. The first symptom anyone saw was the delay itself. The lasting fix was to change the machinery. Equal shares gave way to work stealing, machines that ran dry pulling work from the ones running behind, and a slow node could no longer hold up the run. Out-of-sync nodes were now flagged for a person to investigate later, and the system coped in the meantime.
