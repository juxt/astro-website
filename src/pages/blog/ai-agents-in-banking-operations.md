...
Valuation jobs ran as phases, each phase a set of tasks dealt out round robin through a cluster of messaging nodes to a huge pool of workers across the estate. One node passing its work on a little more slowly, or one machine behind it slowed by a hardware fault, and that share fell behind. Fan-out turned one slow link into everyone's problem. With thousands of jobs in flight, each fanning out thousands of tasks, every job had some task that landed on the slow node, so every job was late. And the system could only run so many jobs at once, so the late ones sat holding capacity and everything queued behind them waited. Queueing theory is blunt about systems like that. Run one close to full utilisation and a small delay grows out of all proportion to its cause. Nothing had failed, so nothing alerted. The first symptom anyone saw was the delay itself. The lasting fix was to change the machinery. Equal shares gave way to work stealing, machines that ran dry pulling work from the ones running behind, and a slow node could no longer hold up the run. Out-of-sync nodes were now flagged for a person to investigate later, and the system coped in the meantime.
