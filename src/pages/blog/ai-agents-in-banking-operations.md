...
Picture a busy kitchen. Every order is split among the cooks, and the dishes are dealt out in strict turn, so each cook gets an equal share. Now let one cook be slightly slow. Every order has a dish waiting on that cook, so every table eats late. Tables stay occupied longer, so the queue at the door grows too. The fix, once you see it, is simple. Keep dealing the dishes in turn, but let a cook who has finished their own help out the one running behind.

We ran that kitchen at scale. Valuation jobs arrived in their thousands, each split into tens of thousands of tasks, dealt out in strict turn through a cluster of messaging nodes to a huge pool of machines. One node passing work on a little more slowly, or one machine behind it slowed by a hardware fault, and every job had a dish waiting on the slow cook. Nothing had failed, so nothing alerted. The first symptom anyone saw was the delay itself. The lasting fix was the one from the kitchen. The grid moved to work stealing, machines that ran dry pulling work from the ones running behind, and a slow node could no longer hold up the run. Out-of-sync nodes were now flagged for a person to investigate later, and the system coped in the meantime.
