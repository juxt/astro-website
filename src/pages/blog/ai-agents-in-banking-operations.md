...
The case that stuck with me involved a messaging node whose performance drifted, only slightly, under heavy load. Queueing theory did the rest. Its backlog grew quietly, every job in the batch had tasks waiting on it, and nothing had failed, so nothing alerted. The first symptom was users asking why their calculations were taking longer than usual. The lasting fix was nodes that periodically stepped in for a peer falling behind, and queue depths watched continuously, so drift reached a person before a user could feel it.
