...
Picture a busy kitchen. Every order is split among the cooks, and the dishes are dealt out in strict turn, so each cook gets an equal share. Now let one cook be slightly slow. Every order has a dish waiting on that cook, so every table eats late. Tables stay occupied longer, so the queue at the door grows too. The fix, once you see it, is to stop dealing in turn. An idle cook picks up the next dish, whoever's turn it was.
