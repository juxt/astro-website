...
export function single_quadrant_visualization(
  config: SingleQuadrantConfig
): void {
  ...
  config.entries.forEach((entry) => {
    ...
    const minRadius =
      entry.ring === 0
        ? circleRadius + buffer
        : prevRing.radius + circleRadius + buffer
  ...
  // Draw blip shapes based on moved property
  blips.each(function (d) {
    const blip = d3.select(this)

    if (d.moved == 1) {
      // Triangle pointing up
      blip.append('path').attr('d', 'M -9,4 9,4 0,-10 z').style('fill', d.color)
    } else if (d.moved == -1) {
      // Triangle pointing down
      blip
        .append('path')
        .attr('d', 'M -9,-4 9,-4 0,10 z')
        .style('fill', d.color)
    } else if (d.moved == 2) {
      // Star for new entries
      blip
        .append('path')
  ...
  function updateBlipPositions() {
    ...
    config.entries.forEach((entry) => {
      ...
      const clampedRadius = Math.max(
        minRadius,
        Math.min(maxRadius, currentRadius)
      )
